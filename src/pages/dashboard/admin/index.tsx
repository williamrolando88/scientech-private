...
const ExportExpenses: FC = () => {
  ...
  const customPayment2Purchase = (data: any): Purchase => {
  ...
  const invoice2Purchase = (data: any): Purchase => {
    const ref: Record<string, string> = {};
  ...
  const saleNote2Purchase = (data: any): Purchase => {
    const ref: Record<string, string> = {};
  ...
  const nonDeductible2Purchase = (data: any): Purchase => {
    const ref: Record<string, string> = {};
...
const getRefMapper = (oldData: any[]) => {
  ...
  oldData.forEach((data) =>
    data.transactions.forEach(async (details: any) => {
      if (!data.id) return;
...
const ExportDoubleEntryAccounting: FC = () => {
  ...
  const fetchOldRecords = async () => {
    ...
    const q = query(collection(DB, 'dayBookTransactions'));

    const querySnapshot = await getDocs(q);

    const oldDataArray = querySnapshot.docs.map((d) => d.data() as any);
